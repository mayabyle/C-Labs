...
void kill_virus(FILE* file, int byteLocation, int signatureSize) {    //CHANGED ARGUMENT TO FILE* INSTEAD OF char*
	fseek(file, byteLocation,SEEK_SET);
	char *del = (char*)calloc(signatureSize , sizeof(char));
	for(int i=0; i<signatureSize; i++)
		del[i]=nop;
	fwrite(del, 1, signatureSize, file);
	free(del);
}
...
link* fix_file(link* list, FILE* file, char *fileName) {
	...
	scanf("%hd",&byteLocation);
	printf("Enter Virus size:\n");
	scanf("%hd",&signatureSize);
